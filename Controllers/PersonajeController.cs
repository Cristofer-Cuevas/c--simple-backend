...
public class PersonajeController : ControllerBase
{
    ...
    [HttpDelete("{id}")]
    public async Task<ActionResult<PersonajeModel>> Delete ([FromRoute] int id) {
		PersonajeModel personajeModel = dbContext.Personaje.Where((p) => p.Id == id).FirstOrDefault();



        try {
			await dbContext.Personaje.FindAsync(id);
            if (personajeModel == null) return NotFound("Element with the passed ID was not found");
			dbContext.Personaje.Remove(personajeModel);
			await dbContext.SaveChangesAsync();
		} catch (System.Exception) {
			return Problem("An error has occured");
		}
        return Ok("Success Deleting the personaje");
	}
}
